...
quint32 getUInt32FromByteArray(const QByteArray &data, bool bigEndian = false){
    ...
    quint8 d2 = data[2];
    quint8 d3 = data[3];
    if(!bigEndian) ret = static_cast<quint32>(d0)
            + (static_cast<quint32>(d1) << 8)
            + (static_cast<quint32>(d2) << 16)
            + (static_cast<quint32>(d3) << 24);
    else ret = static_cast<quint32>(d3)
            + (static_cast<quint32>(d2) << 8)
            + (static_cast<quint32>(d1) << 16)
            + (static_cast<quint32>(d0) << 24);*/
...
quint16 getUInt16FromByteArray(const QByteArray& data, bool bigEndian = false){
    ...
    /*quint8 d0 = data[0];
    quint8 d1 = data[1];
    if(!bigEndian) ret = static_cast<quint16>(d0)
            + (static_cast<quint16>(d1) << 8);
    else ret = static_cast<quint16>(d1)
            + (static_cast<quint16>(d0) << 8);*/
    const char* dataPtr = data.data();
...
bool RIFFWaveEditor::open(QFile &wavefile){
    ...
    // check file header
    if(!(data.mid(0, 4) == "RIFF") || !(data.mid(8, 4) == "WAVE")) return false;
...
bool RIFFWaveEditor::checkApplicability(const QByteArray &formatChunk){
    ...
    return formatChunk[0] == 0x01 && formatChunk[1] == 0x00 // it should be Linear PCM Format
            ...
            && (getUInt32FromByteArray(formatChunk.mid(4,4)) * 4 == getUInt32FromByteArray(formatChunk.mid(8,4))) // validity check
...
//        this->lChannel.append(getInt16FromByteArray(dataChunk.mid(idx, 2)));
//        this->rChannel.append(getInt16FromByteArray(dataChunk.mid(idx + 2, 2)));
        this->lChannelFloat.append(static_cast<float>(getInt16FromByteArray(dataChunk.mid(idx, 2))) / 32768.0f);
        this->rChannelFloat.append(static_cast<float>(getInt16FromByteArray(dataChunk.mid(idx + 2, 2))) / 32768.0f);
    }
}
...
//        dataChunkDest.append((reinterpret_cast<const char*>(lChannel.data() + idx)), 2);
//        dataChunkDest.append((reinterpret_cast<const char*>(rChannel.data() + idx)), 2);
        qint16 lSample = static_cast<qint16>(qRound(lChannelFloat[idx] * 32767.0f));
        qint16 rSample = static_cast<qint16>(qRound(rChannelFloat[idx] * 32767.0f));
...
void RIFFWaveEditor::mixAt(RIFFWaveEditor& source, quint32 offsetSample){
    ...
    if(overlapSample >= 0){
        // if overlap samples overcede source samples, appending rest of source is not needed
        ...
        int iterationRange = overlapSample < source.lChannelFloat.size() ? overlapSample : source.lChannelFloat.size();
        for(int olidx = 0; olidx < iterationRange; olidx++){
            lChannelFloat[offsetSample + olidx] = lChannelFloat[offsetSample + olidx] + source.lChannelFloat[olidx];
            rChannelFloat[offsetSample + olidx] = rChannelFloat[offsetSample + olidx] + source.rChannelFloat[olidx];
        }
    }
...
float RIFFWaveEditor::getMaxAmplitude(){
    ...
    float max = 0.0f;
    for(quint32 idx = 0; idx < len; idx++){
        max = (qAbs(this->lChannelFloat[idx]) > max) ? qAbs(this->lChannelFloat[idx]) : max;
        max = (qAbs(this->rChannelFloat[idx]) > max) ? qAbs(this->rChannelFloat[idx]) : max;
    }
