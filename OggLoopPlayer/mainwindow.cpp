    ...
    : QMainWindow(parent)
{
	...
	seekPositionSelector->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
	...
	cullentTimeLabel->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Preferred);
	cullentFileNameLabel = new QLabel("---", this);
	cullentFileNameLabel->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Preferred);
...
//        if(!player->open(QFile::encodeName(cullentFileName).data())){
        if(!player->open(cullentFileName)){
            QMessageBox::warning(this, "Cannot open file",
                                 QString("Couldn't open file: ") + cullentFileName + "\nReason : " + player->getErrorReason(),
		...
		int totalSecond = player->getAllSampleNum() / player->getAudioFormat().sampleRate();
		cullentTimeText.sprintf("0:00/%d:%02d", totalSecond / 60, totalSecond % 60);
...
void MainWindow::seek(){
	...
	player->seekTo(seekPositionSelector->value());
...
void MainWindow::changeCullentPlayPosition(){
	...
	QString cullentTimeText = "";
	cullentTimeText.sprintf("%d:%02d/%d:%02d",cullentSecond / 60, cullentSecond % 60, totalSecond / 60, totalSecond / 60);
	cullentTimeLabel->setText(cullentTimeText);
